---
title: "ADP Blob Storage Usage"
---

# Blob Storage Backend (Progressive Disclosure)

Apply this guidance when `adp_discover()` returns `"backend:BLOB_STORAGE"` in the resource's `tags`
(e.g., `demo:documents`).

**Strict enforcement:** The server rejects any request with fields or operations not
in the usage contract. Always build intents from `adp_describe()` output.
