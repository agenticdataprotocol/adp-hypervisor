...
def _make_resolver(
    ...
    """Create a RoleResolver with the given config."""
    config = UserRoleConfig(default_role=default_role, users=users or {})
    return config, RoleResolver(config)
...
class TestRoleResolver(unittest.TestCase):
    """Tests for RoleResolver.resolve() covering auth parsing and role lookup."""
    ...
    def test_valid_basic_auth(self) -> None:
        """Valid Basic Auth resolves to the mapped role."""
    ...
    def test_no_meta(self) -> None:
        """Params with no _meta key returns default role."""
        self.assertEqual(self.resolver.resolve({}), "default")

    def test_empty_meta(self) -> None:
        """Empty _meta dict returns default role."""
        self.assertEqual(self.resolver.resolve({"_meta": {}}), "default")

    def test_no_authorization(self) -> None:
        """_meta without authorization key returns default role."""
        self.assertEqual(self.resolver.resolve({"_meta": {"other": "val"}}), "default")

    def test_empty_authorization(self) -> None:
        """Empty authorization string returns default role."""
        params = _params_with_auth("")
        self.assertEqual(self.resolver.resolve(params), "default")

    def test_non_string_authorization(self) -> None:
        """Non-string authorization value returns default role."""
        self.assertEqual(self.resolver.resolve({"_meta": {"authorization": 123}}), "default")

    def test_non_dict_meta(self) -> None:
        """Non-dict _meta value returns default role."""
        self.assertEqual(self.resolver.resolve({"_meta": "not a dict"}), "default")

    def test_invalid_scheme(self) -> None:
        """Non-Basic scheme returns default role."""
        params = _params_with_auth("Bearer token123")
        self.assertEqual(self.resolver.resolve(params), "default")

    def test_invalid_base64(self) -> None:
        """Invalid base64 payload returns default role."""
    ...
    def test_empty_username(self) -> None:
        """Empty username (colon-prefixed) returns default role."""
